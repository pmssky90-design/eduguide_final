...
SITE_URL = BASE_URL
THUMBNAIL_IMAGE_DIR = ROOT / "images" / "thumbnails"
THUMBNAIL_IMAGE_URL_BASE = f"{BASE_URL}/images/thumbnails"
THUMBNAIL_IMAGE_SRC_BASE = "/images/thumbnails"
...
def thumbnail_name_for_canonical(canonical: str) -> str:
    digest = hashlib.sha256(canonical.encode("utf-8")).digest()
    index = int.from_bytes(digest, "big") % len(THUMBNAIL_IMAGE_NAMES)
    return THUMBNAIL_IMAGE_NAMES[index]


def thumbnail_image_url(name: str) -> str:
    return f"{THUMBNAIL_IMAGE_URL_BASE}/{name}"


def thumbnail_image_src(name: str) -> str:
...
def render_page(page, all_pages, regions):
    ...
    selected_image_name = thumbnail_name_for_canonical(canonical)
    selected_image = thumbnail_image_url(selected_image_name)
...
def render_home(content_pages, region_pages, regions):
    ...
    home_image_name = thumbnail_name_for_canonical(canonical)
